...
def fetch_naver_news(keyword: str):
    """네이버 뉴스 크롤링 (주 1회)"""
    try:
        url = f"https://search.naver.com/search.naver?where=news&query={requests.utils.quote(keyword)}&sort=1&pd=4"
        headers = {
        ...
        results = []
        for item in soup.select(".news_area")[:6]:
            title_el = item.select_one(".news_tit")
            info_el  = item.select_one(".info_group .info")
            if title_el:
                results.append({
        ...
        return results, datetime.datetime.now(KST).strftime("%Y-%m-%d %H:%M")
...
def fetch_ftc_news(keyword: str):
    """공정위 보도자료 크롤링 (주 1회)"""
    try:
        url = f"https://www.ftc.go.kr/www/selectReportUserView.do?key=10&rpttype=1&searchKey={requests.utils.quote(keyword)}"
        ...
        soup = BeautifulSoup(resp.text, "html.parser")
        results = []
        for row in soup.select("table tbody tr")[:4]:
            cols = row.select("td")
            if len(cols) >= 3:
                title_el = row.select_one("a")
                date_el  = cols[-1] if cols else None
                if title_el:
                    results.append({
                        "title": title_el.get_text(strip=True),
                        "link": "https://www.ftc.go.kr" + title_el.get("href","#"),
...
def fetch_mfds_news():
    ...
    try:
        ...
        soup = BeautifulSoup(resp.text, "html.parser")
        results = []
        for row in soup.select(".board_list tbody tr")[:4]:
            title_el = row.select_one("td.left a, td a")
            date_el  = row.select_one("td:last-child")
            if title_el:
...
with tab2:
    ...
    with st.spinner("뉴스 데이터를 불러오는 중..."):
        ...
        mfds_list, mfds_ts  = fetch_mfds_news()
    ...
    with c2:
        # 공정위 + 식약처
        all_press = (ftc_list or []) + (mfds_list or [])
        badge2 = f'<span class="src-badge crawl">🕷️ 공정위/식약처 크롤링 · {ftc_ts or mfds_ts or "미수집"}</span>'
        if all_press:
            press_rows = "".join([
                f'<div class="press">'
